...
int main(int argc, char const *argv[]){
	...
	for(i = MIN_PLAYERS; i < (MAX_PLAYERS + 1); i++){
		for (j = 1; j < 10; j++){ //initializes 9 game states for each possible number of players
			...
			/* ---- Salvager Card ---- */
			testNumBuys = game.numBuys; //Get base game->numBuys to compare later
			...
			if (testValue == -1){ //Ensure cardEffect routes the card correctly.
				...
				if (game.numBuys != (testNumBuys + 1)){ //outpostCard works correctly.
					//printf("Error: cardtest2 [game->numBuys] is an unexpected value.\n");
					printf("testNumBuys[%d], game.numBuys[%d]\n", testNumBuys, game.numBuys);
				}
				testCoins = testCoins + getCost( handCard(myChoice1, &game) ); //returns the value of the card selected and adds it to the testCard
				if (testCoins != game.coins)
					//printf("Error: cardtest2 [game->coins] is an unexpected value.\n");
